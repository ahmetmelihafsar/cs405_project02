...
class MeshDrawer {
	...
	setMesh(vertPos, texCoords, normalCoords) {
		...
		// MY CODE STARTS
		// Bind and set normal coordinates
	...
	draw(trans) {
		...
		// gl.uniform3fv(this.lightSourceLoc, lightPosWorld);

		// Set light position uniform
		// var lightPos = [lightX, lightY, 1.0]; // Make sure this is correctly set
...
const meshFS = `
			...
			void main()
			{
				if(showTex && enableLighting){
       				...
       				float diffuse = dot(normal, lightPos); // Simple diffuse lighting
					gl_FragColor = max(texture2D(tex, v_texCoord) * (ambient), texture2D(tex, v_texCoord) * (diffuse + ambient));
				}
				else if(showTex){
...
var lightY = 1;
var lightZ = 1;
var lightPosWorld = [lightX, lightY, lightZ]; // Assuming a Z position of 1.0
...
function updateLightPos() {
	...
	// they were working reversed, so I applied a fix below
	if (keys['ArrowUp']) lightY += translationSpeed;
	if (keys['ArrowDown']) lightY -= translationSpeed;
	if (keys['ArrowRight']) lightX += translationSpeed;
	if (keys['ArrowLeft']) lightX -= translationSpeed;
	lightPosWorld = [lightX, lightY, lightZ]; // Update the global light position

	// print to log
	console.log("Light position: " + lightPosWorld);
